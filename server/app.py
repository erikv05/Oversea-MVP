...
def generate_email():
    try:
        ...
        prompt += "and remember to make the email as personal as possible.\nThe email must be less than 200 words."

        completion = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": f"You are a skilled email writer on the marketing team named {marketer_name} working to write simple and easy-to-read personalized emails."},
                {"role": "user", "content": prompt}
            ],
        )
        return jsonify({'email': completion.choices[0].message.content}), 200
